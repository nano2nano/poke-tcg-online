...
describe("席の引き取り", () => {
   ...
   * 入れ替えればチケットは全部知らないものになる。これを「降りている」と答えると、ウィンドウを 1 つしか
   * 開いていない人にまで「別のウィンドウから入り直した」と言うことになる。
   */
  it("知らないチケットは「降りている」ではなく「知らない」と答える", () => {
    ...
    // こちらから降ろしたチケットだけが「降りている」になる。同じプレイヤーが別のウィンドウから入り直す形。
    const deck = legalDecks()[0]!;
    const { secret } = arena.accounts.create("ふたつのウィンドウ", 0);

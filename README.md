...
- **打ち手について、識別子と表示名と持ち点より多くを保存しない。** メールアドレスもパスワードも
  置かない。学習に要るのは「同じ人か」と「どのくらい強いか」だけである。
  合言葉は控えを持たず、公開の識別子とは別の値にする（同じ値にすると対局ログが合言葉の一覧になる）。
- **カードの名前から `defId` を推測しない。** 同じ名前で定義が違うカードが多い（名前 1,470 種のうち
  455 種、定義の 53.3%）。`src/decklist.ts` は選べないときに候補を返して拒否する。
  黙って 1 つ選ぶと、例外も出ないまま別のデッキで対戦が始まる。
